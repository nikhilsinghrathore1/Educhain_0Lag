...
export default function TaskContract() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className="w-full justify-start text-left font-normal border-gray-200 dark:border-slate-700 dark:bg-slate-800"
              disabled={isLoading}
            >
      ...
      <Button
        className={`w-full gap-2 ${getThemeValue("primary")}`}
      ...
      {txHash && (
        <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
          <p className="text-sm text-emerald-700 dark:text-emerald-300 font-medium">Transaction submitted!</p>
          <p className="text-xs text-emerald-600 dark:text-emerald-400 font-mono break-all">{txHash}</p>
          <a
            href={`https://sepolia.etherscan.io/tx/${txHash}`}
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs text-emerald-700 dark:text-emerald-300 underline mt-2 inline-block"
          >
            View on Etherscan
          </a>
        </div>
